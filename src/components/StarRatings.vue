<template>
  <div class="star-rating">
    <div v-for="(star, index) in stars" :key="index" class="star-container">
      ...
      >
        <polygon :points="getStarPoints" style="fill-rule:nonzero;stroke:#ff523b;stroke-width:1" />
        <defs>
          <!--
...
export default {
  name: 'starsRatings',
  components: {},
  props: ['productRatings'],
  data() {
    return {
      config: {
        rating: this.productRatings,
    ...
    setStars() {
      ...
      for (let i = 0; i < this.stars.length; i++) {
        if (fullStarsCounter !== 0) {
          ...
          this.stars[i].raw = Math.round(surplus * 10) / 10;
          this.stars[i].percent = this.calcStarFullness(this.stars[i]);
          return this.stars[i].percent;
        }
      }
    },
    setConfigData() {
